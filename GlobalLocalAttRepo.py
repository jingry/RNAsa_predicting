...
class selfEncodeBlock(nn.Module):
    def __init__(self,oneHotDimOut,headNum,dropout_rate,NFusioniter,device):
        ...
        self.attFusion = seqFusionAttentionWeightProjCP(oneHotDimOut,headNum=headNum,dropout_rate=dropout_rate,device=device,localWindowSize=1)
        self.attFusion1 = seqFusionAttentionWeightProjCP(oneHotDimOut,headNum=headNum,dropout_rate=dropout_rate,device=device,localWindowSize=3)
        self.attFusion2 = seqFusionAttentionWeightProjCP(oneHotDimOut,headNum=headNum,dropout_rate=dropout_rate,device=device,localWindowSize=5)
...
class selfEncodeConvBlock(nn.Module):
    def __init__(self,oneHotDimOut,headNum,dropout_rate,NFusioniter,device):
        ...
        self.fusionlayerNorm1 = nn.LayerNorm(oneHotDimOut)
        self.fusionlayerNorm_o1 = nn.LayerNorm(oneHotDimOut)
        self.attFusion = seqFusionAttentionWeightProj(oneHotDimOut,headNum=headNum,dropout_rate=dropout_rate,device=device,localWindowSize=10)
        self.attFusion1 = seqFusionAttentionWeightProj(oneHotDimOut,headNum=headNum,dropout_rate=dropout_rate,device=device,localWindowSize=20)
    ...
    def forward(self,fusionArr,seqMask=None):
        for iterFusion in range(self.NFusioniter):
            tmpfusionArr = fusionArr.transpose(-2,-1)
            ...
            fusionArr = (tmpfusionArr + self.conv3(tmpfusionArr)).transpose(-2,-1)
            tmpfusionArr = self.fusionlayerNorm(fusionArr)
            fusionArr = fusionArr + self.attFusion(tmpfusionArr,tmpfusionArr,tmpfusionArr,seqMask =seqMask)
            fusionArr = fusionArr + self.fusionFF(fusionArr)
        return fusionArr
...
class myModel(nn.Module):
    ...
    def forward(self, oneHotOriArrIn, covMats, posEncMats, bool_parameter=False, optimizer=None,predict=False,preTrain=False, fillNum=0):
        ...
        oneHotEbdArr = self.OneHotlayerNorm(self.onehotEbd(oneHotArr))+posEncMats  
        graphEbdArrLast = self.LastSteplayerNorm(self.graphEbdLast(graphArrLast))+posEncMats 
        graphEbdArrNext = self.NextSteplayerNorm(self.graphEbdNext(graphArrNext))+posEncMats  

        fusionArr = self.onehotEbdConv(oneHotArr)
        
        asa_max = torch.einsum('d,nds->ns',self.asa_std,oneHotOriArrIn)

        tmpArr = self.selfEncodeBlock(fusionArr)
